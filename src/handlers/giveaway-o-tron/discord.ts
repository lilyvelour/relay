...
export async function sendMessage(guildId: string, channelId: string, message: MessageData) {
  try {
    ...
    const channel = (await guild.channels.fetch(channelId)) as ActualChannelType
    ...
    await channel.send({
      embeds: [new EmbedBuilder().setColor(message.colour).setTitle(title).setURL(message.link).setDescription(body)],
    })
    console.log('[discord][message][sent]', { guildId, channelId, message })
  } catch (e) {
    console.error('[discord][message][error]', { guildId, channelId, message }, e.message)
  }
}
